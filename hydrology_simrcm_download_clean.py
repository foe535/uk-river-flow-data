"""
eFLaG Hydrology Bulk Download and Clean (simrcm) — Freshwater Digital Twin project

Separate from hydrology_download_clean.py (which handles simobs) on
purpose, simrcm is a genuinely different product: driven by 12 different
future-climate projections rather than one real historical record, over
a longer period extending into the 2080s. Its column structure has not
been confirmed to match simobs, run 'test' first and actually look at
the result (the script prints the columns it finds) before trusting
'clean' output for anything.

Works for any of the four eFLaG models (G2G, GR4J, GR6J, PDM), same
station list as the simobs script.
...
USAGE:
    python3 hydrology_simrcm_download_clean.py GR6J test       # 3 stations, LOOK at the columns printed
    python3 hydrology_simrcm_download_clean.py GR6J download    # all 200, only after test looks right
    python3 hydrology_simrcm_download_clean.py GR6J clean       # combine into one CSV

Each model gets its own folder and output file (e.g. gr6j_simrcm_raw/
and gr6j_simrcm_all_catchments_clean.csv), kept separate from the
simobs files the other script produces, nothing overwrites anything.
"""
...
def base_url(model: str) -> str:
    return f"https://catalogue.ceh.ac.uk/datastore/eidchub/1bb90673-ad37-4679-90b9-0126109639a9/River_Flow/{model}/simrcm"


def raw_dir(model: str) -> Path:
    d = Path(os.environ.get(f"{model}_SIMRCM_RAW_DIR", Path(__file__).parent / f"{model.lower()}_simrcm_raw"))
...
def output_file(model: str) -> Path:
    return Path(
        os.environ.get(
            f"{model}_SIMRCM_OUTPUT_FILE",
            Path(__file__).parent / f"{model.lower()}_simrcm_all_catchments_clean.csv",
        )
    )


def download_one(model: str, station: int) -> str:
    url = f"{base_url(model)}/{model}_simrcm_{station}.csv"
    dest = raw_dir(model) / f"{model}_simrcm_{station}.csv"
...
def download_batch(model: str, stations: list[int]):
    ...
    for i, station in enumerate(stations, 1):
        ...
        print(f"[{i}/{len(stations)}] {model} simrcm station {station}: {result}")
    ...
    print(f"\n{ok_count}/{len(stations)} {model} simrcm files available in {raw_dir(model)}")
    ...
    if ok_count > 0:
        # Show the actual columns from whichever file just downloaded, since
        # simrcm's structure hasn't been confirmed, don't assume it matches simobs.
        any_station = next((s for s in stations if (raw_dir(model) / f"{model}_simrcm_{s}.csv").exists()), None)
        if any_station:
            sample = pd.read_csv(raw_dir(model) / f"{model}_simrcm_{any_station}.csv")
            print(f"\nColumns found in station {any_station}'s file: {list(sample.columns)}")
...
def clean_and_combine(model: str):
    ...
    for station in STATIONS:
        f = raw_dir(model) / f"{model}_simrcm_{station}.csv"
        if not f.exists():
    ...
    if not frames:
        sys.exit(f"No downloaded {model} simrcm files found. Run 'download' first.")

    combined = pd.concat(frames, ignore_index=True)
    out = output_file(model)
    combined.to_csv(out, index=False)

    print(f"Combined {len(frames)} {model} simrcm stations, {len(combined)} total rows.")
